...
local SilentAimEnabled = true -- Cámbialo a false si quieres apagarlo al inicio

-- FUNCIÓN PARA VOLVERSE INVISIBLE (MÉTODO GHOST)
local function ActivateGhost()
    local char = localPlayer.Character
    if char then
        local root = char:FindFirstChild("HumanoidRootPart")
        if root then
            -- 1. Engañamos al servidor eliminando el root original
            local clone = root:Clone()
            clone.Parent = char
            root:Destroy() 
            
            -- 2. Transparencia local para saber que estás en modo fantasma
...
local function getTarget()
    ...
    local dist = 1000
    for _, p in pairs(Players:GetPlayers()) do
        -- Filtro: Que no seas tú, que sea de otro equipo y que esté vivo
        if p ~= localPlayer and p.Team ~= localPlayer.Team and p.Character then
            local torso = p.Character:FindFirstChild("UpperTorso") or p.Character:FindFirstChild("HumanoidRootPart")
            local hum = p.Character:FindFirstChildOfClass("Humanoid")
            if torso and hum and hum.Health > 0 then
                local d = (localPlayer.Character.HumanoidRootPart.Position - torso.Position).Magnitude
                if d < dist then
                    dist = d
                    target = torso
                end
...
-- MOTOR DE DAÑO AL TORSO (SILENT AIM)
RunService.Heartbeat:Connect(function()
    if SilentAimEnabled then
        ...
        if tool then
            ...
            if enemy then
                -- Forzar daño al torso del enemigo vía Remotos
                for _, v in pairs(tool:GetDescendants()) do
                    if v:IsA("RemoteEvent") then
                        v:FireServer(enemy.Position, enemy)
                    end
                end
                -- Contacto para el cuchillo (FireTouch)
                local handle = tool:FindFirstChild("Handle") or tool:FindFirstChildOfClass("Part")
                if handle then
                    firetouchinterest(enemy, handle, 0)
                    firetouchinterest(enemy, handle, 1)
                end
            end
        end
    end
...
-- EJECUCIÓN AUTOMÁTICA DE LA INVISIBILIDAD
ActivateGhost()
